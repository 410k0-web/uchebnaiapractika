...
CREATE TABLE IF NOT EXISTS ProductApplications (
    ...
    CreatedProductId INTEGER NULL,
    FOREIGN KEY (UserId) REFERENCES Users(UserId) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (ProcessedByUserId) REFERENCES Users(UserId) ON UPDATE CASCADE ON DELETE SET NULL,
    FOREIGN KEY (ProductTypeId) REFERENCES ProductTypes(ProductTypeId) ON UPDATE CASCADE ON DELETE RESTRICT,
    FOREIGN KEY (MaterialTypeId) REFERENCES MaterialTypes(MaterialTypeId) ON UPDATE CASCADE ON DELETE RESTRICT,
    FOREIGN KEY (CreatedProductId) REFERENCES Products(ProductId) ON UPDATE CASCADE ON DELETE SET NULL
);

CREATE INDEX IF NOT EXISTS IX_Users_Email ON Users(Email);
CREATE INDEX IF NOT EXISTS IX_ProductApplications_UserId ON ProductApplications(UserId);
CREATE INDEX IF NOT EXISTS IX_ProductApplications_Status ON ProductApplications(Status);
CREATE INDEX IF NOT EXISTS IX_ProductApplications_ProductTypeId ON ProductApplications(ProductTypeId);
CREATE INDEX IF NOT EXISTS IX_ProductApplications_MaterialTypeId ON ProductApplications(MaterialTypeId);

-- Тестовые учетные записи создаются программно при запуске приложения:
-- admin@comfort.local / Admin123!
-- user@comfort.local / User123!
